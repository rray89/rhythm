...
- 数据概览窗口：可从菜单打开独立窗口，查看 `今日`、`最近 7 天`、`最近 30 天`、`全部历史` 汇总、分组 session history，以及按范围导出
...
- 数据记录：保存专注 / 休息片段、计划时长、实际时长、结束原因，并按周写入本地 `Application Support/Rhythm/history/weeks/` JSON 历史目录；数据概览窗口会按统计日分组展示记录，并支持把 `今日`、`最近 7 天`、`最近 30 天`、`全部历史` 导出为 CSV 或 JSON；应用关闭恢复状态写入 `Application Support/Rhythm/state/app-lifecycle.json`
...
- 数据概览快照、隐藏休息历史状态与按范围 CSV / JSON 导出
