...
- 菜单栏应用：常驻状态栏，保留图标并实时显示当前倒计时，快速查看状态与最近记录；所有窗口关闭时，隐藏的数据概览窗口与已关闭的菜单面板都不会跟随每秒倒计时反复刷新；同时避免两个 Rhythm 副本同时运行，包括后续从本地构建再次启动的重复副本
...
- 数据概览快照、单快照展示状态、隐藏休息历史状态与固定范围 / 所选日期 CSV / JSON 导出
- 专注即将结束通知
