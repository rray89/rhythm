...
- Menu bar app: stays in the status bar, keeps the icon visible, shows a live countdown for quick status checks and recent history, keeps hidden Insights work off the per-second countdown path while windows are closed, and prevents two Rhythm copies from running at the same time, including later duplicate launches from local builds
