...
- Menu bar app: stays in the status bar, keeps the icon visible, shows a live countdown for quick status checks and recent history, keeps hidden Insights work and the closed menu panel off the per-second countdown path while windows are closed, and prevents two Rhythm copies from running at the same time, including later duplicate launches from local builds
...
- insights snapshots, one-snapshot presentation state, hidden-rest history state, and fixed-range / selected-day CSV/JSON export
